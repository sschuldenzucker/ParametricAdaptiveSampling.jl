...
"Interval of t values, with function values and midpoint. Ensures t1 < t2 but we don't check this on construction. V must be a tuple type."
struct Segment{V}
    t1::Float64
    v1::V
    t_mid::Float64
...
### Keyword Arguments
- `errfun`: Error function `(Segment, Range) -> Float64` where the `Range` is (an estimate of) the value range. Default = [`err_relative_range`](@ref) = l-infinity distance component-wise relative to range; this is a good default for plotting.
...
Non-parametric variant of [`sample_adaptive_parametric`](@ref) for a function `f: Float64 -> Real`. See the documentation of that function. The same keyword arguments are supported.

### Returns

`x-y pairs :: Vector{Tuple{Real, Real}}`. These are the resulting points.

Note that the return format is different from [`sample_adaptive_parametric`](@ref) because there is no separate space of "t values" here.
